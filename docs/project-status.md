...
> 최종 업데이트: 2026-05-23 (smoke test 추가)  
...
Agent MVP가 LangGraph 위에서 동작하고, 각 노드 흐름이 로그로 추적된다. 통합 smoke test로 실서비스 의존(OpenAI/Mongo/Chroma)까지 한 번에 검증할 수 있다.
...
### 우선순위 5. `app/data/models/` → 도메인 `models.py`로 이동
