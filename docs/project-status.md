...
- [x] `GET /student/learning/records/me` — 학습 기록 조회
...
- [x] `tests/agent/test_chat_integration.py` — `RUN_INTEGRATION_TESTS=1`일 때만 활성화
...
- `POST /student/learning/stage1/submit-card-check` — 카드 쌍 정답 평가 + LearningRecord 저장
- `POST /student/learning/stage2/submit-answer` — 문제 답안 평가 + LearningRecord 저장
...
`tests/agent/test_chat_integration.py` 추가.
...
RUN_INTEGRATION_TESTS=1 pytest tests/agent/test_chat_integration.py -q
